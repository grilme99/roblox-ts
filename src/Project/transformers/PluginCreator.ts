...
import { addDiagnosticFactory } from "Project/transformers/util/addDiagnosticFactory";
...
const requireStack: Array<string> = [];
/**
 * @example
 *
 * new PluginCreator([
...
export class PluginCreator {
	...
	mergeTransformers(into: TransformerList, source: ts.CustomTransformers | TransformerBasePlugin) {
		const slice = <T>(input: T | Array<T>) => (Array.isArray(input) ? input.slice() : [input]);
		if (source.before) {
	...
	private resolveFactory(transform: string, importKey = "default"): PluginFactory | undefined {
		if (
			!tsNodeIncluded &&
			transform.match(/\.tsx?$/) &&
	...
	private validateConfigs(configs: Array<PluginConfig>) {
